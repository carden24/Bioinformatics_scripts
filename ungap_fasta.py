...
def create_an_inputs_and_output(input_file):
   ...
   shortname = re.sub('[.](fasta$|fa$|faa$|fna$)','',input_file, re.I)
   output_file = shortname+"_ungapped.fasta"
...
def main(argv):
   ...
   for record in SeqIO.parse(filein,"fasta"):
      ...
      sequence = sequence.replace('.', '')
      fileout.write('>%s %s\n%s\n' % (record.name, record.description, sequence))
   filein.close()
   fileout.close()
